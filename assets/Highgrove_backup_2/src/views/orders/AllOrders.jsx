...
import { OrderDetails, orderDelete, GetOrderDetails, fetchLogs, cartId, pushOrders, fetchOrderDataToPush } from '../api/api';
...
function AllOrders() {
  ...
  const LoadOrderDetails = async () => {
    ...
    //   const modifiedData = result.data.map((item, index) => ({
    //     ...item, No: index + 1 , 'Order Number' : item.order_num, 'Order Created Date': moment(item.creation_date).format('yyyy-MM-DD'), 'Status': item.status === 0 ? 'Open' : 'Confirmed', 'Customer Code' : item.customer_code, 'Customer Name' : item.customer_name, 'Amount' : '£ '+ item.amount, 'Created By Agent' : item.first_name, 'Promised Delivery Date' : moment(item.promise_delivery_date).format('yyyy-MM-DD')
    //  }));
    //  modifiedData.forEach(item => {
    //    delete item.id;
    //    delete item.product_id;
  ...
  const handleFilter = (e) => {
    if (e.target.value == '') {
      // setorderDetails(orderSearch)
      setsorttableData(orderSearch)
    } else {
      // const filterResult = orderSearch.filter(item => item.order_num && item.order_num.toLowerCase().includes(e.target.value.toLowerCase()) || item.creation_date && item.creation_date.toString().includes(e.target.value.toString()) || item.status && item.status.toString().includes(e.target.value.toString()) || item.customer_code && item.customer_code.toLowerCase().includes(e.target.value.toLowerCase()) || item.customer_name && item.customer_name.toLowerCase().includes(e.target.value.toLowerCase()) || item.amount && item.amount.toString().includes(e.target.value.toString()) || item.first_name && item.first_name.toLowerCase().includes(e.target.value.toLowerCase()) || item.delivery_address && item.delivery_address.toLowerCase().includes(e.target.value.toLowerCase()) || item.promise_delivery_date && item.promise_delivery_date.toLowerCase().includes(e.target.value.toLowerCase()) || item.customer_doc_num && item.customer_doc_num.toLowerCase().includes(e.target.value.toLowerCase()) )
        ...
        <div className="mb-4 px-2 users-table-div">
          <CCardHeader className='users-table-title users-table-inner-div'>
            ...
            <div className='userdetail-head'>
              ...
              <div className='top-btngroup'>
                {roleauth === 'Agent' ? null : <button className='top-button' onClick={() => { FetchDataToPush() }}>Push To Sage</button>}

                <button className='top-button' onClick={() => setVisible(!visible)}>Filter</button>
                <CModal alignment="center" className='date-filter' visible={visible} onClose={() => setVisible(false)} backdrop="static">
                  ...
                  <CModalFooter>
                    <CButton onClick={() => { setVisible(false); setStartDate(null); setEndDate(null); setProDate(null); }} className='white-btn'>
                      Close
                    </CButton>
              ...
              <div className='export-grp'>
                <p>Export</p>&nbsp;&nbsp;
                <img id="test-table-xls-button" alt="excel" title="Excel" className="download-table-xls-button" src={Excel} width={30} onClick={() => exportExcelData()} />
                <button onClick={downloadPdf} id='test-table-pdf-button' className='pdf-btn' ><img src={PdfIcon} alt="pdf" title="PDF" width={30} /></button>
              </div>
            </div>
          </CCardHeader>
              ...
              <CTable id="table-to-xls" ref={ref} className='users-table orders-table'>
                  ...
                  <CTableRow>
                    ...
                    <CTableHeaderCell scope="col" className='table-head-th'>
                      <span className='th_text'>Amount&nbsp;&nbsp;
                        <div style={{ display: 'grid' }}>
                          <img src={AscendingIcon} height={25} onClick={() => sortDataAsc('total_amount')} className='sortin-icon' />
                          <img src={DescendingIcon} height={25} onClick={() => sortDataDesc('total_amount')} className='sortin-icon' />
                        </div>
                      </span>
                    </CTableHeaderCell>
                    <CTableHeaderCell scope="col" className='table-head-th'>
                ...
                <CTableBody>
                  {sorttableData && currentData.map((orders, index) => {
                    // order creation date formate change
                    const formattedDate = moment(orders.creation_date).format('DD-MMM-YYYY');
                    return (
                      <CTableRow key={orders.id} className='dashboardtablesdatarow'>
                        ...
                        {roleauth === 'Agent' ? null :
                          <CTableDataCell className='tbl-col-action'>
                            ...
                            <button className={"action-btn " + (orders.status == '0' ? 'hidden-btn' : '')} title='View order' onClick={() => ViewOrderRoute(orders.portal_ref_num, orders.id)}><img src={ViewOrdersIcon} /></button>
                            <button className='action-btn' title='order Logs' onClick={() => openModal(orders.id)}><img src={ViewPopupIcon} /></button>
                            <button value={orders.id} style={{ backgroundColor: 'transparent', border: 'none', padding: '0px', margin: '0px' }} disabled={orders.status != '0'} onClick={() => { deleteOrderPopup(orders) }}>
                              <img id={orders.id} src={DeletIcon} alt='delete order' title='Delete order' />
                            </button>
                            {showModal ?
        ...
        <div className="model-div delete-popup" ref={modalRef} onClick={closeModal}>
          <div className="modal">
            ...
            <div className='model-popup-btn-group'>
              ...
              <button className="AddUserBtn save-btn" onClick={() => deleteOrder()} id="deleteTooltip" >Delete</button>
